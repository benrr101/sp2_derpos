	...
	while(1) {
		...
		if(strncmp(command, "touch", 20) == 0) {
			...
			if(f == NULL) {
				...
				printf("*** fopen failed with code 0x%x\n");
				continue;
			}
			...
			if(strlen(filename) != 10 || filename[1] != ':') {
				printf("*** Invalid filename. X:YYYYYYYY %d\n", strlen(filename));
				continue;
			}
			...
			for(i = 0; i < ata_device_count; i++) {
				printf("Drive %d: %s 0x%x sectors (512b)\n",
					i, 
					ata_devices[i].model,
					ata_devices[i].size
					);
			...
			if(result != FS_SUCCESS) {
				printf("*** Partition failed with code 0x%x\n");
			}	
...
void user_l( void ) {
	...
	if( status != SUCCESS ) {
		...
		printf( "User L exiting at time %u\n", time );
	}
	exit();
...
void user_p( void ) {
	...
	for( i = 0; i < 3; ++i ) {
		...
		if( status != SUCCESS ) {
			...
			printf( "User P reporting time %08x\n", time );
		}
		write( 'P' );
	}

	printf( "User P exiting\n" );
	exit();
...
void user_t( void ) {
	...
	if( status != SUCCESS ) {
		...
		printf( "User T (%d) running, initial prio %d\n",
		  	  pid, priority );
	}
	...
	if( status != SUCCESS ) {
		...
		printf( "User T, prio was %d now %d\n", priority, prio2 );
	}
	priority = prio2;
